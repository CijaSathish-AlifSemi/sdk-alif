...
static uint16_t glps_seq_num;
static uint16_t glps_store_idx;
static int glps_current_index;

static uint16_t glps_send_idx = 0;
static uint16_t glps_nb_stored;
...
static void on_glps_meas_send_complete(uint8_t conidx, uint16_t status)
{
    ...
    /* Keep ability to send (CCCD gate) */
    glps_ready_to_send = true;

    if (glps_nb_to_send == 0) {
        /* Done: respond to RACP using the SAME opcode we received */
                            ...
                            const union glp_filter *p_filter)
{
    ...
    /* Start a NEW transfer from record 0 */
    glps_send_idx = 0;
    ...
    /* If nothing to send (or notifications not enabled), respond properly */
...
static uint16_t set_advertising_data(uint8_t actv_idx)
{
    ...
    memcpy(p_data + 14, &svc_lls,  2);
    memcpy(p_data + 16, &svc_batt, 2);
...
static void glps_store_measurement(void)
{
    prf_date_time_t *time = (prf_date_time_t *)get_device_time();

    prf_sfloat value = glps_convert_to_sfloat(70 + (glps_current_index++ % 30));
    ...
    /* IMPORTANT:
     * glps_store_idx = "number of stored records"
     * Do NOT wrap it to 0, or you'll tell RACP there are 0 records.
     */
    uint16_t idx;
    if (glps_store_idx < GLPS_STORE_MAX) {
        idx = glps_store_idx;
        glps_store_idx++;
    } else {
        /* storage full: overwrite last slot (minimal change, no ring-buffer rewrite) */
        idx = GLPS_STORE_MAX - 1;
    }

    glps_ext_meas[idx].p_meas = meas;
    glps_ext_meas[idx].ext_seq_num = glps_seq_num++;

    glps_available_data = true;
}
